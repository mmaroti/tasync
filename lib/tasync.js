...
(function () {
	...
	function InvokeFuture (func, that, args, index) {
		Future.call(this);
	...
	InvokeFuture.prototype = Object.create(Future.prototype);

	InvokeFuture.prototype.getPath = function () {
	...
	InvokeFuture.prototype.onRejected = function (error) {
		this.args = null;
		this.reject(error);
	};

	InvokeFuture.prototype.onResolved = function invoke_on_resolved_trace_start (value) {
		assert(this.state === STATE_LISTEN);
		...
		for (index = 0; index < args.length; ++index) {
			...
			if (value instanceof Future) {
				if (value.state === STATE_RESOLVED) {
					...
					var future = new InvokeFuture(func, that, args, index);
					value.register(future);
					return future;
				} else {
					assert(value.state === STATE_REJECTED);
	...
	function ThenFuture (func, that, value) {
		InvokeFuture.call(this, func, that, [ null, value ], 1);
	}

	ThenFuture.prototype = Object.create(InvokeFuture.prototype);
	...
	ThrottleFuture.prototype.getPath = InvokeFuture.prototype.getPath;
